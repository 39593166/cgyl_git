...
public class OrderMannerFragment extends NetDataBaseFragment implements
		OnRefreshListener2<ExpandableListView>, OnChildClickListener,
		OnGroupClickListener {
	OrderMannergerActivity context;
	View pView;
	...
	public static OrderMannerFragment getInstance(int state) {
		OrderMannerFragment instance = new OrderMannerFragment();
		instance.state = state;
		return instance;
	}

	// public OrderMannerFragment(int state) {
	// super();
	// this.state = state;
	// }
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		context = (OrderMannergerActivity) getActivity();
	}
	...
	private void getOrders(int tag) {
		...
		// Log.e("userId", userId + "");
		apply(API_F.getOrderList(userId, sid, state, pageIndex, true, tag));
	}
	...
	public boolean onChildClick(ExpandableListView parent, View v,
		...
		intent.putExtra("goodsId",
				orderAdapter.getChild(groupPosition, childPosition).getId());
	...
	public boolean onGroupClick(ExpandableListView parent, View v,
		...
		Intent intent = new Intent();
		intent.setClass(getActivity(), ShopOrderInfoActivity.class);
