...
public class ShopOrderInfoActivity extends NetDataBaseActionBarActivity
	...
	private static final int TAG_UPDATEORDER_3 = 11;
	int orderId;
	CgOrderId order;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_order_info_dp);
		initToolbar("查看订单");
	...
	private Button orderInfoSetAppend;
	...
	private void findViews() {
		...
		orderInfoSetAppend = (Button) findViewById(R.id.orderInfoSetAppend);
		...
		orderInfoSetAppend.setOnClickListener(this);
	...
	public void onClick(View v) {
		if (v == orderInfoSetAppend) {
			...
			View view = this.getLayoutInflater().inflate(
					R.layout.dialog_orderdp_inputwnum, null);
			builder.setView(view);
			final EditText editText = (EditText) view
					.findViewById(R.id.addOrderWnum);
			builder.setTitle("请输入物流单号以供买家查询");
			builder.setPositiveButton("确定",
					new DialogInterface.OnClickListener() {
						@Override
						public void onClick(DialogInterface dialog, int which) {
							String wnum = editText.getText().toString().trim();
							updateOrder(wnum);
						}
					});
		...
		public void handleMessage(Message msg) {
			...
			if (msg.what == MSG_WHAT) {
				...
				if (TextUtils.equals(resultStatus, "9000")) {
					Toast.makeText(ShopOrderInfoActivity.this, "支付成功",
							Toast.LENGTH_SHORT).show();
				} else {
					// 判断resultStatus 为非“9000”则代表可能支付失败
					// “8000”代表支付结果因为支付渠道原因或者系统原因还在等待支付结果确认，最终交易是否成功以服务端异步通知为准（小概率状态）
					if (TextUtils.equals(resultStatus, "8000")) {
						Toast.makeText(ShopOrderInfoActivity.this, "支付结果确认中",
								Toast.LENGTH_SHORT).show();

					} else {
						// 其他值就可以判断为支付失败，包括用户主动取消支付，或者系统返回的错误
						Toast.makeText(ShopOrderInfoActivity.this, "支付失败",
								Toast.LENGTH_SHORT).show();
		...
		if (TAG == TAG_GETORDER) {
			...
			try {
				...
				if (state == 1) {
					...
					orderInfoSetAppend.setVisibility(View.GONE);
				}
			} catch (JSONException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
	...
	private void setOrderType() {
		...
		if (order.getNowState() == CgOrderId.STATU_PAYED) {
			orderInfoStatu.setText("已发货");
			orderInfoSetAppend.setVisibility(View.VISIBLE);
		}
	}
			...
			orderInfoGoodsInfo
					.setOnItemClickListener(new OnItemClickListener() {
						...
						public void onItemClick(AdapterView<?> parent,
							...
							intent.setClass(ShopOrderInfoActivity.this,
	...
	private void updateOrder(String wnum) {
		order.setNowState(CgOrderId.STATU_APPENDDING);
		order.setWnum(wnum);
		order.setGoodslist(null);
		apply(API_F.updateOrder(order, TAG_UPDATEORDER_3, userNeed.getId(),
				userNeed.getSid()));
	}
